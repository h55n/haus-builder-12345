...
const DESIGN_REQUEST_TIMEOUT_MS = 180000
...
export function useArchitectAgent() {
  ...
  const refineDesign = useCallback(async (profile: UserProfile, instruction: string): Promise<DesignSpec | null> => {
    ...
    const controller = new AbortController()
    const timeoutId = window.setTimeout(
      () => controller.abort(new Error(`Design refinement request timed out after ${DESIGN_REQUEST_TIMEOUT_MS / 1000} seconds`)),
      DESIGN_REQUEST_TIMEOUT_MS
    )
    try {
      const res = await fetch('/api/agent/design', {
      ...
      const msg = e instanceof Error ? e.message : String(e)
      console.error('[refineDesign]', msg)
      const fallback = buildFallbackDesign(profile)
      setDesign(fallback)
      setError(null)
      return fallback
